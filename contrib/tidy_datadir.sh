...
if [ -d "$1" ]; then
  ...
  echo "Removes obsolete Insifa database files" >&2
  exit 1
fi
...
case $LEVEL in
  0)
    echo "Error: no Insifa datadir detected."
    exit 1
    ;;
  1)
    echo "Detected old Insifa datadir (before 0.7)."
    echo "Nothing to do."
    exit 0
    ;;
  2)
    echo "Detected Insifa 0.7 datadir."
    ;;
  3)
    echo "Detected Insifa pre-0.8 datadir."
    ;;
  4)
    echo "Detected Insifa 0.8 datadir."
    ;;
esac
