...
*Setup instructions for a gitian build of Insifa using a Debian VM or physical system.*

Gitian is the deterministic build process that is used to build the Insifa
...
- [Building Insifa](#building-insifa)
...
Clone the git repositories for insifa and gitian and then checkout the insifa version that you want to build.
...
Follow the instructions in [doc/release-process.md](release-process.md) in the insifa repository
...
To build Insifa (for Linux, OSX and Windows) just follow the steps under 'perform
gitian builds' in [doc/release-process.md](release-process.md) in the insifa repository.
...
```bash
    Initialized empty Git repository in /home/debian/gitian-builder/inputs/insifa/.git/
    remote: Reusing existing pack: 35606, done.
    remote: Total 35606 (delta 0), reused 0 (delta 0)
    Receiving objects: 100% (35606/35606), 26.52 MiB | 4.28 MiB/s, done.
    Resolving deltas: 100% (25724/25724), done.
    From https://github.com/Insifa/insifa
...
For example:
```bash
URL=https://github.com/konez2k/insifa.git
COMMIT=b616fb8ef0d49a919b72b0388b091aaec5849b96
./bin/gbuild --commit insifa=${COMMIT} --url insifa=${URL} ../insifa/contrib/gitian-descriptors/gitian-linux.yml
./bin/gbuild --commit insifa=${COMMIT} --url insifa=${URL} ../insifa/contrib/gitian-descriptors/gitian-win.yml
./bin/gbuild --commit insifa=${COMMIT} --url insifa=${URL} ../insifa/contrib/gitian-descriptors/gitian-osx.yml
```
...
```bash
    gpg --detach-sign ${VERSION}-linux/${SIGNER}/insifa-build.assert
    gpg --detach-sign ${VERSION}-win/${SIGNER}/insifa-build.assert
    gpg --detach-sign ${VERSION}-osx/${SIGNER}/insifa-build.assert
```
...
[insifa/gitian.sigs](https://github.com/Insifa/gitian.sigs/) repository, or if that's not possible to create a pull
request.
